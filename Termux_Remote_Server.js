...
const os = require('os'); // IP Address හොයාගැනීමට
        ...
        for (const name of Object.keys(nets)) {
            for (const net of nets[name]) {
                if (net.family === 'IPv4' && !net.internal) {
                    if (name.startsWith('rmnet') || name.startsWith('ccmni')) mobileIp = net.address; // Mobile Data IP
                    else if (name === 'wlan0') wifiIp = net.address; // WiFi IP
...
function handleExecute(req, res) {
    ...
    // නව Feature: myip කමාන්ඩ් එක (Internet Access IP පෙන්වයි)
    if (command === 'myip') {
        try { return res.json({ status: 'success', stdout: execSync("curl -s ifconfig.me 2>/dev/null").toString().trim() }); } 
        catch(e) { return res.json({ status: 'success', stdout: 'No Internet Access' }); }
    }
...
wss.on('connection', (ws, req) => {
    ...
    const streamScreen = async () => {
        ...
        setTimeout(streamScreen, 500); // 2 FPS (Speed එක වැඩි කරන්න අවුරුම 100 කරන්න)
    };
    streamScreen();
...
app.get('/screen-ui', (req, res) => {
    ...
    <script>
    let TOKEN = localStorage.getItem('remote_token') || prompt("Enter Auth Token:");
